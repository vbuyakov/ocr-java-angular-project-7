...
curl -sf -u "elastic:${ELASTIC_PASSWORD}" -X PUT "http://elasticsearch:9200/_security/role/logstash_writer" -H "Content-Type: application/json" -d '{"cluster":["monitor","manage_index_templates"],"indices":[{"names":["ocr-ja7-logs*"],"privileges":["all"]}]}'
